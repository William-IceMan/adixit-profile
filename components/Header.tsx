  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        {/* Mobile Nav Overlay */}
        <div className={`fixed inset-0 bg-light-navy flex flex-col items-center justify-center gap-8 transition-all duration-300 md:hidden ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href}
              ...
              >
                <span className="text-lightest-slate font-sans text-xl font-semibold group-hover:text-green">{link.name}</span>
              </a>
            ))}
            <a 
              href={PROFILE.linkedinUrl}
              className="px-8 py-3 mt-4 text-green border border-green rounded font-mono hover:bg-green/10"
            >
              Resume / LinkedIn
            </a>
        </div>
